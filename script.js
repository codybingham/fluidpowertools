  ...
  const GUIDE_MANIFEST_PATH = 'guides_manifest.json';
  const guidesRoot = document.getElementById('guidesContainer');
  if (guidesRoot) {
    loadGuides(guidesRoot);
  }

  async function loadGuides(container) {
    container.innerHTML =
      '<p class="guide-loading">Loading guides…</p>';
    try {
      const res = await fetch(GUIDE_MANIFEST_PATH, {
        cache: 'no-store'
      });
      ...
      if (!guides.length) {
        container.innerHTML =
          '<p class="guide-loading">Guides will appear here after the next build.</p>';
        return;
      }
      container.innerHTML = '';
      if (data.generatedAt) {
      ...
      for (const guide of guides) {
        ...
        link.textContent = guide.title || guide.slug;
      ...
      container.innerHTML =
        '<p class="guide-error">Unable to load guides right now.</p>';
    }
  }
